<template>
  <div>
    <v-dialog v-model="dialog" width="500" class="dialog">
      <v-btn slot="activator" color="red lighten-2" dark fab @click="addData">
        <v-icon>add</v-icon>
      </v-btn>

      <v-card>
        <v-card-title class="headline dialog-head black">
        ...
        <v-card-actions>
          ...
          <v-btn
            ...
            color="error"
            dark
            v-show="dialogData.index===-1 ? false : true"
            @click="cancle"
          >取消修改</v-btn>
        </v-card-actions>
      </v-card>
...
export default class TestDialog extends Vue {
  ...
  private async saveData() {
    ...
    if (this.dialogData.index != -1) {
      //  编辑状态
      this.dataList[this.dialogData.index].title = await this.dialogData
        .content;
